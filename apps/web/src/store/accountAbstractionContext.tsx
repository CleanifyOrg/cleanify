    ...
    useEffect(() => {
        (async () => {
            const options: Web3AuthOptions = {
                clientId: envConfig.web3AuthClientId ?? "",
                web3AuthNetwork: import.meta.env.PROD
                    ? "sapphire_mainnet"
                    : "testnet",
                chainConfig: {
    ...
    useEffect(() => {
        const getSafeAddress = async () => {
            if (web3Provider) {
                ...
                const key: string = envConfig.gelatoRelayApiKey;
            ...
            if (web3Provider) {
                ...
                for (let i = 0; i < 30; i++) {
                    ...
                    if (status?.lastCheckMessage?.includes("Execution error")) {
                        throw new Error(
                            `Transaction failed: ${JSON.stringify(status)}`
                        );
                    }
