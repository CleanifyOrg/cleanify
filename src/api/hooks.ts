...
const getIsSafeDeployedQueryKey = (safeAddress: string, provider?: providers.Web3Provider) => [
    "IS_SAFE_DEPLOYED",
    safeAddress,
 ...
 * @returns  true if the provided address is a contract
 */
export const useIsSafeDeployed = (safeAddress: string, provider?: providers.Web3Provider) => {
    return useQuery({
        queryKey: getIsSafeDeployedQueryKey(safeAddress, provider), queryFn: () =>
            isContractAddress(safeAddress, provider)
    }
    )
}
...
export const useSafeInfo = (safeAddress: string, connectedChainId: string) => {
    return useQuery({
        queryKey: getSafeInfoQueryKey(safeAddress, connectedChainId), queryFn: () =>
            getSafeInfo(safeAddress, connectedChainId)
    }
    )
}